...
postfix_expression:
  ...
  | postfix_expression '(' ')'                           { $$=new_ast_subtree(NO_TYPE, NULL, yylineno, FUNCTION_CALL_NODE, 1, $1); }
...
conditional_expression:
  ...
  | logical_or_expression '?' expression ':' conditional_expression { $$=new_ast_subtree(NO_TYPE, NULL, yylineno, TERN_OP_NOPE, 3, $1, $3, $5); }
;
...
declaration:
  ...
  | declaration_specifiers init_declarator_list ';' { $$=$2; if($1!=NULL){$$=add_ast_child(set_ast_kind($1, STRUCT_VAR_DECL_NODE), $2);} }
...
init_declarator_list:
    init_declarator                          { $$=new_ast_subtree(last_decl_type, NULL, yylineno, INIT_DECL_LIST_NODE, 1, $1); }
  | init_declarator_list ',' init_declarator { $$=add_ast_child($1, $3); }
;

init_declarator:
    declarator '=' initializer { $$=new_ast_subtree(last_decl_type, NULL, yylineno, VAR_DECL_INIT_NODE, 2, $1, $3); }
  | declarator                 { $$=$1; }
;
...
type_specifier:
  ...
  | struct_or_union_specifier { last_decl_type=STRUCT_TYPE; }
  | enum_specifier            { /* TODO */ }
...
struct_or_union_specifier:
    struct_or_union '{' struct_declaration_list '}'             /* Ignorado exceto se usarmos typedef */
  | struct_or_union struct_name '{' struct_declaration_list '}' { $$=new_ast_subtree(STRUCT_TYPE, NULL, yylineno, STRUCT_DECL_NODE, 2, $2, $4); }
  | struct_or_union struct_name                                 { $$=new_ast_subtree(STRUCT_TYPE, NULL, yylineno, STRUCT_DECL_NODE, 1, $2); }
;

struct_name:
    IDENTIFIER { $$=new_ast(STRUCT_TYPE, last_id, yylineno, VAR_DECL_NODE, 0); }
;
...
struct_declarator_list:
    struct_declarator                             { $$=new_ast_subtree(last_decl_type, NULL, yylineno, INIT_DECL_LIST_NODE, 1, $1); }
  | struct_declarator_list ',' struct_declarator  { $$=add_ast_child($1, $3); }
;
...
declarator:
  ...
  | direct_declarator         { $$=$1; if(last_param_list){$$=new_ast_subtree(last_decl_type, NULL, yylineno, FUNCTION_DECL_NODE, 2, $1, last_param_list); last_param_list=NULL;} }
;
...
function_definition:
  ...
  | declaration_specifiers declarator compound_statement                  { $$=new_ast_subtree(get_ast_type($2), NULL, yylineno, FUNCTION_DEF_NODE, 2, $2, $3); }
;
